...
void computeJ(int nE, int nLeq, int nNeq, double *incLoop, double *incNode, double *x, double n, double *tmp, double *res){
    ...
    memcpy(res+offset, incNode, nNeq * nE * sizeof(double));
}
void solve(int nE, int nLeq, int nNeq, double *incLoop, double *conLoop, double *incNode, double *conNode, double *&x, double n, int maxiter = 100000, int maxattempts = 1000){
    int i, j, k;
    ...
    for(i = 0; i < maxattempts; ++i){ // has multiple attempts
        ...
        for(j = 0; j < maxiter; ++j){
            ...
            // check
            if((k = all_zero(nE, R, 1e-6)) > 0) break;
            if(all_zero(nE, R, 1e8) == 0){ k = 3; break; } // too large
            // debug
            // printf("J = \n");   print_A(nE, J);
            // printf("\nR = ");   print_x(nE, R);
            // printf("\ndx = ");  print_x(nE, dx); 
